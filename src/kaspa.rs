                ...
                if let Some(Ok(Message::Text(res))) = ws_stream.next().await {
                    if let Ok(parsed) = serde_json::from_str::<Value>(&res) {
                        if let Some(result) = parsed.get("result") {
                            if let Some(blues) = result.get("block").and_then(|b| b.get("verboseData")).and_then(|v| v.get("mergeSetBluesHashes")).and_then(|h| h.as_array()) {
...
pub async fn start_kaspa_engine(state: Arc<AppState>, bot: Bot) {
    ...
    loop {
        ...
        let mut request = match ws_url.clone().into_client_request() {
            Ok(req) => req,
            Err(e) => { log::error!("❌ [NODE] Invalid URL: {}", e); tokio::time::sleep(Duration::from_secs(5)).await; continue; }
        };
        request.headers_mut().insert("sec-websocket-protocol", "wrpc-json".parse().unwrap());
                                ...
                                if let Some(method) = parsed.get("method").and_then(|m| m.as_str()) {
                                    if method == "utxosChangedNotification" {
                                        if let Some(params) = parsed.get("params") {
                                            handle_utxos_changed(params, state.clone(), bot.clone(), http_client.clone(), ws_url.clone()).await;
                                        }
                                    }
                                } else if let Some(result) = parsed.get("result") {
                                     log::info!("📥 [NODE ACK] Subscription Successful: {:?}", result);
                                }
                            }
                        }
                        Ok(Message::Close(c)) => { log::warn!("⚠️ [NODE] Closed: {:?}", c); break; }
                        Err(e) => { log::error!("❌ [NODE] WS Error: {}", e); break; }
                        _ => {} 
                    }
                }
            }
            Err(e) => { log::error!("❌ [NODE FATAL] Fail: {}", e); }
        }
        ...
        for entry in added {
            ...
            tokio::spawn(async move {
                ...
                for chat_id in subscribers {
                   let _ = bot_clone.send_message(ChatId(chat_id), build_msg("...", "...", "...")).parse_mode(ParseMode::Markdown).await;
                }
            });
        }
    }
