...
pub enum Command { Start, Help, Add(String), Remove(String), List, Balance, Price, Market, Fees, Network, Supply }
...
fn main_menu() -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        ...
        vec![InlineKeyboardButton::callback("💵 Price", "cmd_price"), InlineKeyboardButton::callback("🌐 Network", "cmd_network")],
        vec![InlineKeyboardButton::callback("⛽ Fees", "cmd_fees"), InlineKeyboardButton::callback("🪙 Supply", "cmd_supply")],
    ])
}
...
async fn handle_plain_text(bot: Bot, msg: Message, state: Arc<AppState>) -> ResponseResult<()> {
    if let Some(text) = msg.text() {
        if let Some(valid) = clean_and_validate_wallet(text) {
            ...
            let entry = state.monitored_wallets.entry(valid.clone()).or_insert_with(|| {
                is_new = true;
            ...
            let txt = if is_new { format!("✅ *Added for tracking:*\n`{}`", valid) } else { format!("⚠️ *Already tracked:*\n`{}`", valid) };
...
async fn handle_cmd(bot: Bot, msg: Message, cmd: Command, state: Arc<AppState>, api: Arc<ApiManager>) -> ResponseResult<()> {
    ...
    match cmd {
        Command::Start | Command::Help => {
            ...
            bot.send_message(msg.chat.id, "🤖 *Kaspa Pulse Bot*\nReal-time Node Monitoring. Choose an option:").parse_mode(ParseMode::Markdown).reply_markup(main_menu()).await?;
        }
        Command::Balance => {
        ...
        Command::Price => {
            ...
            let price = api.get_price().await.map(|v| v["price"].as_f64().unwrap_or(0.0)).unwrap_or(0.0);
            bot.send_message(msg.chat.id, format!("💵 *Current Price:* `${:.4}`", price)).parse_mode(ParseMode::Markdown).await?;
        }
        Command::Network => {
        ...
        Command::Supply => {
            ...
            if let Ok(s) = api.get_supply().await {
                let circ = s["circulatingSupply"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0) / 100_000_000.0;
                bot.send_message(msg.chat.id, format!("🪙 *Circulating Supply:* `{:.0} KAS*`", circ)).parse_mode(ParseMode::Markdown).await?;
            }
        }
        _ => {}
    }
