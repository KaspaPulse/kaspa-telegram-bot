...
pub fn main_menu_markup() -> InlineKeyboardMarkup {
    ...
    let row5 = vec![
        InlineKeyboardButton::callback("❤️ Support & Donate", "cmd_donate"),
    ];
...
pub fn format_hashrate(val: f64) -> String {
    if val <= 0.0 { return "0.00 TH/s".to_string(); }
    if val < 1000.0 { format!("{:.2} TH/s", val) } 
    else if val < 1_000_000.0 { format!("{:.2} PH/s", val / 1000.0) } 
...
#[derive(Deserialize)]
pub struct HashrateResponse { pub hashrate: f64, }

pub async fn fetch_hashrate_api() -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let client = reqwest::Client::builder().timeout(Duration::from_secs(5)).build()?;
    let resp = client.get("https://api.kaspa.org/info/hashrate").send().await?.json::<HashrateResponse>().await?;
